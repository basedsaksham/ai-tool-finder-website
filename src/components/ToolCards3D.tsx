...
const ToolCards3D = ({ tools, onToolClick }: ToolCards3DProps) => {
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      if (containerRef.current) {
        ...
        const speed = Math.max(0.3, Math.min(2, 1 + distance * 1.5));
  ...
  useEffect(() => {
    const animate = () => {
      setAnimationTime(prev => prev + 0.01 * orbitalSpeed); // Smoother increment
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0">
        {/* Orbital paths with fixed animations */}
        <div className="absolute inset-0 opacity-20">
          {[120, 160, 200, 240].map((radius, i) => (
            <div
              key={`orbit-${i}`}
              className="absolute top-1/2 left-1/2 border border-white/10 rounded-full animate-pulse"
              style={{
                width: `${radius * 2}px`,
                height: `${radius * 2}px`,
        ...
        {/* Floating cosmic particles with fixed animations */}
        ...
        <div className="absolute inset-0 overflow-hidden">
          {Array.from({ length: 3 }).map((_, i) => (
            <div
              key={`beam-${i}`}
              className="absolute w-full h-px bg-gradient-to-r from-transparent via-red-500/40 to-transparent animate-pulse"
              style={{
                top: `${25 + i * 25}%`,
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="w-16 h-2 bg-white/20 rounded-full">
            <div 
              ...
              style={{ width: `${Math.min(100, (orbitalSpeed / 2) * 100)}%` }}
            />
          </div>
        </div>
      </div>

      {/* Smooth Orbital System */}
      <div className="absolute inset-0 flex items-center justify-center">
        <div className="relative w-full h-full">
          {visibleTools.map((tool, index) => {
            // Create stable orbital layers
            const orbitConfigs = [
              { radius: 130, speed: 1, phase: 0 },
              { radius: 170, speed: 0.8, phase: 60 },
              { radius: 150, speed: 1.2, phase: 120 },
              { radius: 190, speed: 0.6, phase: 180 },
              { radius: 160, speed: 1.1, phase: 240 },
              { radius: 210, speed: 0.7, phase: 300 },
            ];
            
            const config = orbitConfigs[index] || orbitConfigs[0];
            
            // Smooth orbital calculation
            ...
            const floatY = Math.sin(animationTime * 0.5 + index * 1.5) * 12;
            
            // Smooth depth effects
            const scale = 0.85 + (z + config.radius) / (config.radius * 3);
